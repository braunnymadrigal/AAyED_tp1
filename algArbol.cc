...
void Algoritmos::AlturaNodoPreOrdenR(
    Arbol *A, Arbol::Node n, int &altura, int k) {
...
int Algoritmos::CantNivelesPorNiveles(Arbol *A) {
    ...
    if (!A->Vacio()) {
        k = 1;
        std::vector<std::pair<Arbol::Node, int>> *L =
            new std::vector<std::pair<Arbol::Node, int>>;
...
void Algoritmos::ListarIesimoNivelR(
    Arbol *A, Arbol::Node n, int i, int nivAct) {
    if (nivAct == i) {
